...
- `frontend/` — **Nuxt 4** SSR (Vue 3 + TS + Tailwind 4 + Pinia). Port `3001`. Source in [frontend/app/](../frontend/app/).
...
Endpoints: backend `http://127.0.0.1:8000`, frontend `http://127.0.0.1:3001`, phpMyAdmin `http://127.0.0.1:8080`.
...
- Port allocation is centrally tracked in [axelnova-infra/docs/port-allocation.md](../../axelnova-infra/docs/port-allocation.md) — HoP owns `3001` (frontend) and `8000` (backend). Update that file in the same PR that introduces a new port.
