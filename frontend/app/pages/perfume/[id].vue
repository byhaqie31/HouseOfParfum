  ...
  <div class="min-h-screen pt-20 pb-24 px-6">
    <div class="max-w-5xl mx-auto">
      ...
      <template v-else>
        ...
        <header class="mt-8 grid grid-cols-1 md:grid-cols-[220px_1fr] gap-10 md:gap-14 border-y border-ink relative py-10">
          ...
          <div>
            ...
            <p class="mt-4 font-mono text-[10px] uppercase tracking-[0.16em] text-ink-mute">
              ...
              <template v-if="perfume.year && perfume.size"><span class="text-accent-deep mx-1">·</span></template>
              <template v-if="perfume.size">{{ perfume.size }} ml</template>
              <template v-if="perfume.suit">
                <span class="text-accent-deep mx-1">·</span>{{ perfume.suit }}
              </template>
              <template v-if="perfume.quality">
                <span class="text-accent-deep mx-1">·</span>{{ perfume.quality }}
              </template>
            </p>
        ...
        <!-- Bottom CTA -->
        <section class="mt-20 pt-10 border-t border-rule flex flex-wrap items-center justify-between gap-6">
          <p class="font-display italic text-[15px] text-ink-soft max-w-md">
...
const goBack = () => {
  // Prefer the actual previous route if it was the catalog; otherwise navigate.
